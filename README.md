...
|------|-------------|--------|
| **Part 1** | Data exploration — class distribution, sample images | `outputs/plots/class_distribution.png` |
...
| **Part 4** | Bonus — Streamlit demo app | `streamlit_app/` |
...
|-----------------|---------------|
| **Single Responsibility** | Each module has one job: `transforms.py` (transforms only), `trainer.py` (training only), `components.py` (UI only) |
| **Open/Closed** | New views in `streamlit_app/views/`, new routes in `api/routers/` — no existing code modified |
| **Dependency Inversion** | Streamlit and FastAPI depend on `DiseasePredictor` abstraction, not raw model loading |
...
Production-ready FastAPI with OpenAPI docs, request tracing, and rate limiting:
...
MobileNetV2 is production-ready for farmer-facing deployment: **97.8% accuracy** at **9.3 MB** (reducible to ~3 MB with TFLite INT8 quantization), **~9 ms** GPU inference, and offline-capable via TFLite export. Easily extensible to all 38 PlantVillage classes through additional fine-tuning.

---

*Detailed guides: [`wiki/execution-guide.md`](wiki/execution-guide.md) (end-to-end execution order) | [`wiki/architecture.md`](wiki/architecture.md) (Mermaid architecture diagrams)*
